...
import {
  ...
  // sendToRoom,
  // routineTimer,
} from "@/services/cards/timedGameActions";

database_connection().then(() =>
  console.log("Connected successfully to MongoDB (Play Timed Cards)")
);
...
export async function POST(request) {
  try {
    ...
    switch (action) {
      ...
      case "draw":
        if (data.isPenalty) {
          ...
          console.log("### DRAWING CARDS ###", data.roomId);
    ...
    // // If action was successful and was a timer-triggering action
    // if (timerTriggeringActions.includes(action) && data.roomId) {
    //   // Notify clients about timer start

    //   await sendToRoom(data.roomId, {
    //     timer: "success",
    //     timestamp: Date.now(),
    //     duration: 30000,
    //   });

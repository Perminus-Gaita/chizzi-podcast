...
import { roomAggregationPipeline } from "@/services/cards/aggregations";
import {
...
export const sendToRoom = async (roomId, payload) => {
...
const resetTurnTimer = async (roomId) => {
  if (activeTimeout) {
    clearTimeout(activeTimeout);
  }

  activeTimeout = setTimeout(() => handleTimerExpiration(roomId), 30000);
    ...
    if (allCheckedIn && room.players.length === room.maxPlayers) {
      ...
      await sendToRoom(roomId, {
        ...
        timer: {
          startTime: Date.now(),
          duration: 30000,
          playerId: aggregatedRoom[0].turn._id,
        },
      });
    }
...
export const handleTimerExpiration = async (roomId) => {
  ...
  try {
    ...
    console.log("### RUNNING TIMER NOW ###");

    // Check card count to delete schedule
    if (room.drawPile.length + room.discardPile.length < 5) {
      room.gameStatus = "gameover";
      await room.save();
      console.log("Updated game status to 'gameover'.");
      return;
    }

    // Check card count for reshuffle
    if (room.drawPile.length <= 5 && room.discardPile.length > 5) {
      // Create a new draw pile from the discard pile
    ...
    // Only decrement jumpCounter if this is the jumped player's expired turn
    const lastMove =
      room.gamePlay && room.gamePlay.length > 0
        ? room.gamePlay[room.gamePlay.length - 1]
        : null;

    // Handle jump counter expiration only for player being jumped
    if (
      room.jumpCounter > 0 &&
      lastMove?.moveType === "play" &&
      lastMove?.card.startsWith("J")
    ) {
      room.jumpCounter -= 1;
      room.gamePlay.push({
        player: room.turn,
    ...
    } else if (room.isKickback && lastMove?.card?.startsWith("K")) {
      // Player didn't respond to kickback
    ...
    const currentPlayerIndex = room.players.findIndex(
      (p) => p.player._id.toString() === room.turn._id.toString()
    );

    const currentPlayer = room.players[currentPlayerIndex];

    // room in penalty state
    const isCurrentPlayerPenalized =
      room.isPenalty &&
      room.gamePlay.length > 0 &&
      lastMove?.player.toString() !== currentPlayer.player._id.toString();

    if (isCurrentPlayerPenalized) {
      console.log("### HANDLING PENALTY ###");
      ...
      for (let i = 0; i < penaltyCards; i++) {
        ...
        currentPlayer.playerDeck.push(cardDrawn);
        room.gamePlay.push({
      ...
      await sendToOne(currentPlayer.player._id, {
        playerObj: {
      ...
      // Issue a card from the draw pile as a penalty(not playing/not picking answer card)
      const cardDrawn = room.drawPile.pop(); // Remove a card from the draw pile
      currentPlayer.playerDeck.push(cardDrawn); // Add the drawn card to the player's deck
      ...
      // // Log penalty
      // console.log(
      //   `Player ${currentPlayer.player._id} penalized for not drawing or playing a card. Drew ${cardDrawn}.`
      // );
    }

    // Update room document for next player's turn
    const nextPlayerIndex =
      (currentPlayerIndex + room.direction + room.players.length) %
      room.players.length;

    // Check if next player was on and can finish game
    const nextPlayerHasMove = hasValidMove(
      room.currentSuit,
      room.players[nextPlayerIndex].playerDeck,
      room.discardPile[room.discardPile.length - 1]
    );

    // remove kadi for next player
    ...
    console.error("Error handling turn timer:", error);
  }
};
...
const handlePenalty = async (roomId, userId, cardPlayed) => {
  try {
    ...
    if (!hasMoveAfterPlay && !canFinishGame(currentPlayer.playerDeck)) {
      ...
      const startTime = resetTurnTimer(roomId);
      console.log(`### TIMER RESET ### @ ${startTime}`);
...
const handleAnswer = async (roomId, userId, cardPlayed) => {
  ...
  try {
    ...
    if (isPenalty) {
      ...
      await sendToOne(userId, {
        invalidMove: {
          message: "Cannot block Penalty with Answer Card",
        },
      });
      return;
    }
    ...
    if (isKickback) {
      await sendToOne(userId, {
        invalidMove: {
          message: "Cannot block kickback with Answer Card",
        },
      });
    ...
    if (currentPlayer.playerDeck.length === 0 && currentPlayer.on) {
      ...
      room.gameStatus = "gameover";
      // room.turn = null; // Explicitly set turn to null
      ...
      // Handle tournament or regular game payout
      if (tournamentId) {
        ...
        // Handle regular game payout with atomic transaction
        ...
        // Update room pot
        room.pot = 0; // Reset pot after payout
      }
...
export const dealCards = async (roomId) => {
  try {
    ...
    room.turn = room.players[randomIndex].player;
